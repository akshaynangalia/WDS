...
def test_manual_input_parses_priority_and_calendar():
    data = manual_input_parser.parse(os.path.join(FIXTURES, "Manual_Input.xlsx"))
    assert data.priority is not None and not data.priority.empty
    assert data.calendar is not None and not data.calendar.empty
    assert "Link Code" in data.priority.columns
    assert "Plant" in data.priority.columns
    assert "Line" in data.priority.columns
    assert "MOQ" in data.priority.columns
    assert 1 in data.priority.columns  # per-period priority columns "1".."14"
...
def test_parsers_release_the_workbook_file_handle(tmp_path):
    ...
    for i, (parse_fn, sheet) in enumerate((
        ...
        (manual_input_parser.parse, "Priority(Linkcode Level)"),
    )):
