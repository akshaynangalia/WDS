...
- **Calculation Trace sheet**: after the Run Report, one row per Link Code per period — the inputs that drove the number (FIN, carry-in, throughput, GE %, daily demand), the case that fired, and a one-line explanation such as *Case D: DOS gap 10.0 d x 8.0 T/d = 80.0 T >= MOQ batch 40.0 T -> 80.0 T in Run 1. Run 2 placed 120.0 T. Carry-out 0.0 T.* Filter by Link Code to check any row. The sentence is built from figures the engine stored while it worked (write-only; they never feed a calculation), so it cannot drift from the numbers, and the parts add up exactly to what was produced. **Difference (T)** is the row's real conservation residual, `(produced + carry-out) - (FIN + carry-in)`: quantities are rounded to 0.1 T at each step, so differences up to about 0.2 T are rounding. Where a product would need more than 3 decimals to check on a calculator the sentence says `~` instead of `=`.
